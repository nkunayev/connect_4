    ...
    private void handleMessage(String msg) {
        ...
        if (msg.startsWith(Protocol.BOARD + ":")) {
            boardPanel.updateBoard(
                msg.substring((Protocol.BOARD + ":").length()));
        }
            ...
            statusLabel.setText(
                msg.substring((Protocol.STATUS + ":").length()));
            boardPanel.setInteractive(false);
        }
        else if (msg.startsWith(Protocol.GAME_START + ":")) {
            String text = msg.substring((Protocol.GAME_START + ":").length());
            statusLabel.setText(text);
            boardPanel.setInteractive(true);
            amPlayer1 = text.startsWith("You are Player 1");
            ...
            // Styled win/lose chat
            StyledDocument doc = chatArea.getStyledDocument();
            Style winStyle = doc.getStyle("WIN");
            if (winStyle == null) {
                winStyle = doc.addStyle("WIN", null);
                StyleConstants.setForeground(winStyle, Color.GREEN.darker());
            }
            Style loseStyle = doc.getStyle("LOSE");
            if (loseStyle == null) {
                loseStyle = doc.addStyle("LOSE", null);
                StyleConstants.setForeground(loseStyle, Color.RED.darker());
            }
            try {
                String winner = r.split(" ")[0]; // the username
            ...
            int choice = JOptionPane.showConfirmDialog(frame,
                prompt, "Play Again?", JOptionPane.YES_NO_OPTION);
            ...
            String[] p = msg.substring((Protocol.STATS_RESPONSE + ":").length())
                          .split(",");
